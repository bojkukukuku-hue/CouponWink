  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white font-display tracking-tight">Bài viết & Tin tức</h1>
          <p className="text-slate-500 font-medium">Xây dựng nội dung chuyên sâu để thu hút người dùng và tăng SEO.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
         {[
           { label: "Tổng số bài", value: posts.length, color: "blue", icon: "article" },
           { label: "Đã xuất bản", value: posts.filter(p => p.status === 'Published').length, color: "emerald", icon: "task_alt" },
           { label: "Tổng lượt xem", value: posts.reduce((acc, p) => acc + p.views, 0).toLocaleString(), color: "purple", icon: "visibility" },
         ].map((stat, i) => (
           <div key={i} className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
             <div className="flex justify-between items-start">
               <div className={`size-12 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-400 flex items-center justify-center`}>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                <th className="py-6 px-10 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Tiêu đề bài viết</th>
                <th className="py-6 px-10 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Danh mục</th>
                <th className="py-6 px-10 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Trạng thái</th>
                <th className="py-6 px-10 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Lượt xem</th>
                ...
                <tr key={post.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                  ...
                  <td className="py-6 px-10 text-right">
                    <div className="flex justify-end gap-2">
                      <Link to={`/admin/editor/${post.id}`} className="p-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 hover:text-primary-500 transition shadow-sm">
