  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      const mapped: PostItem[] = (data as any[]).map((p) => ({
        id: p.id,
        title: p.title ?? '',
        // tuỳ schema của bạn: category / category_name / category_id
        category: p.category ?? p.category_name ?? 'General',
        // tuỳ schema: author / author_name
        author: p.author ?? p.author_name ?? 'Admin',
        // ưu tiên published_at, fallback updated_at/created_at
        date: formatDate(p.published_at ?? p.updated_at ?? p.created_at),
        status: p.is_published ? 'Published' : 'Draft',
        // nếu chưa có views thì để 0
        ...
        {stats.map((stat, i) => (
          <div key={i} className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
            <div className="flex justify-between items-start">
              {/* NOTE: className dạng bg-${stat.color}-50 là dynamic, tailwind có thể không build đủ class nếu chưa safelist */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {loading ? (
                <tr>
                  <td className="py-10 px-10 text-slate-500" colSpan={5}>Loading posts…</td>
                </tr>
              ) : posts.length === 0 ? (
                <tr>
                  <td className="py-10 px-10 text-slate-500" colSpan={5}>No posts found.</td>
                </tr>
              ) : (
                posts.map((post) => (
