...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const allStores = MockDB.getStores().filter((s: any) => s.status === 'Active');
    const settings = MockDB.getSettings();
    const hpSettings = settings?.homepage || { trendingCount: 6, categoryCount: 8, dealsCount: 3, trendingSort: 'featured' };
    
    setHomepageSettings(hpSettings);
    
    // Sorting logic for categories
    let sortedCategories = [...MockDB.getCategories()];
    if (hpSettings.categorySort === 'latest') {
      sortedCategories.sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
    } else if (hpSettings.categorySort === 'clicks') {
      sortedCategories.sort((a, b) => (b.clicks || 0) - (a.clicks || 0));
    } else {
      sortedCategories.sort((a, b) => a.name.localeCompare(b.name));
    }
    setCategories(sortedCategories);

    // Sorting logic for deals (coupons)
    let sortedCoupons = MockDB.getCoupons().filter((c: any) => c.status === 'Active');
    if (hpSettings.dealsSort === 'usage') {
      sortedCoupons.sort((a, b) => (b.usage || 0) - (a.usage || 0));
    } else {
      sortedCoupons.sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
    }
    setCoupons(sortedCoupons);
    
    if (settings?.metaTitle) document.title = settings.metaTitle;

    // Sorting logic for trending stores
    let sortedStores = [...allStores];
    if (hpSettings.trendingSort === 'latest') {
      sortedStores.sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
    } else if (hpSettings.trendingSort === 'clicks') {
      sortedStores.sort((a, b) => (b.clicks || 0) - (a.clicks || 0));
    } else {
      ...
      <LazySection className="py-20 bg-slate-50 dark:bg-slate-900/30">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {stores.slice(0, homepageSettings.trendingCount).map((store) => (
      ...
      <LazySection className="py-20 bg-white dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {categories.slice(0, homepageSettings.categoryCount).map((cat, i) => (
      ...
      <LazySection className="py-20 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {coupons.slice(0, homepageSettings.dealsCount).map((deal, i) => (
