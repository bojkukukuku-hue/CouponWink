...
const HomePage: React.FC = () => {
  ...
  // Tạm thời để rỗng: bước sau mình sẽ nối stores + categories từ Supabase
  const [stores, setStores] = useState<any[]>([]);
  const [categories, setCategories] = useState<any[]>([]);

  // Coupons lấy từ Supabase
  ...
  const handleShowCode = (deal: any) => {
    ...
    setSelectedCoupon({
      storeName: store?.name || deal.storeName || deal.storeId || 'Store',
      title: deal.title,
      code: deal.code,
    ...
    <Layout>
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-8 text-[11px] font-black text-slate-400 uppercase tracking-widest">
            <div className="flex items-center gap-2">
              <span className="material-icons-round text-primary-500 text-sm">check_circle</span> Verified codes
            </div>
            <div className="flex items-center gap-2">
              <span className="material-icons-round text-primary-500 text-sm">update</span> Updated daily
            </div>
            <div className="flex items-center gap-2">
              <span className="material-icons-round text-primary-500 text-sm">person</span> 100% free to use
            </div>
          </div>
        </div>
      </section>
      ...
      <LazySection className="py-20 bg-slate-50 dark:bg-slate-900/30">
        <div className="max-w-7xl mx-auto px-4">
          ...
          {/* NOTE: bước kế tiếp mình sẽ nối stores thật từ Supabase */}
          {stores.length === 0 ? (
            <div className="text-slate-500 text-sm">Stores will appear here after we connect the Stores table.</div>
          ) : (
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mt-2 leading-relaxed">{store.description}</p>
                    </div>
                  </div>
      ...
      <LazySection className="py-20 bg-white dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-4">
          ...
          {/* NOTE: bước kế tiếp mình sẽ nối categories thật từ Supabase */}
          {categories.length === 0 ? (
            <div className="text-slate-500 text-sm">Categories will appear here after we connect the Categories table.</div>
          ) : (
              ...
              {coupons.slice(0, 3).map((deal, i) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-lg font-black text-primary-500">{deal.label ?? deal.discount_label ?? ''}</div>
                  </div>
